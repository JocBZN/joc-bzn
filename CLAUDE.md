...
**Mărimea, în trei trepte, toate în aceeași zi.** Întâi a rămas cea a polițistului (`scale` 1.5, `stop_dist` 41), fiindcă desenele sunt cât se poate de egale — măsurat, nu ghicit: 36×61 px din pânza de 128 la Skinny, 37×61 din cea de 124 la polițist. Apoi „fa-i pe astia cu 1.5x mai mari" → `scale` **2.25** (1.5 × 1.5). Apoi „fa politistu skinny putin de tot mai mic" → **2.1**, unde a și rămas. Tot cel mai mare inamic obișnuit din joc.

**`stop_dist` merge după el: 41 → 54 → 53**, altfel desenul i-ar intra peste player: lățimea pe ecran a sărit de la ~54 px la ~76, deci jumătatea lui + jumătatea player-ului (15) dă ~53. **54 e plafonul dur**, nu o rotunjire: `enemy.gd::_oprire()` taie `stop_dist` la `contact_range` (60) − `STOP_MARGINE` (6) și te avertizează în consolă, fiindcă un inamic care se oprește mai departe decât brațul player-ului n-ar mai apuca niciodată să-l lovească. Deci Skinny nu poate crește mult peste 2.25 fără să urci întâi `contact_range`-ul player-ului.
