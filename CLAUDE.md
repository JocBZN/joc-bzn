...
- **Listele CHOOSE WEAPON / CHOOSE CHARACTER se strâng singure** (din 2026-09-26): o armă sau un caracter nou se adaugă doar în `WEAPONS` / `CHARACTERS`, iar rândurile încap în `LISTA_H` (`_celula_lista`). Pe la al 9-lea rând apare un `push_warning`: de acolo e nevoie de scroll sau de două coloane. Orice schimbare de layout se verifică pe o POZĂ a meniului adevărat (1152×648): pagina trebuie să aibă aer sus și jos.
- **Regulă de design (Răzvan, 2026-09-24): reflectul NU blochează, doar întoarce damage-ul. SINGURUL item care blochează e Mike's Hedgehog.** Un item nou de reflect se pune pe `reflect_pct` (lovitura te atinge oricum); nu-l face să sară peste `take_damage`. Verifică cu `tool_reflect.tscn`.
...
**Fișiere noi:** `tool_coace_cursor.gd/.tscn` (unealtă), `menu/Mouse_anim.png` (bandă de 6 cadre de 52×59, generată), `cursor_clic.gd` (scânteia). **Atinse:** `gamepad.gd` (secțiunea CURSORUL + `_input` + `_process`), `tool_cursor.gd`. Tot aici intră în git `menu/Mouse.png` și `.import`-ul lui: stăteau netrecute în git de la commit-ul din 09-24, deși codul le folosea.

- **Înclinat ~15° spre stânga**, cu metoda RotSprite (Scale2x ×3, apoi rotire și eșantionare cu pixelul cel mai apropiat). Se face o singură dată în unealtă, fiindcă durează prea mult pentru pornirea jocului. `Mouse.png` rămâne desenul-sursă, drept.
- **Clicul:** apăsat = mâna se strânge, se mai apleacă și se întunecă (1-2-3, ține pe 3 cât ții butonul); când dai drumul, sare înapoi cu resort (4-5-0). Durează câte 28 ms pe cadru. Rămâne cursor hardware și doar i se schimbă poza, deci nu apare lag.
- **Vârful `(11, 1)` e același în toate cadrele** (unealta rotește în jurul lui și îl tipărește). Dacă re-coci cadrele, copiază-l în `CURSOR_VARF`.
- **Scânteia de aramă** pornește din vârful degetului, doar cât se vede mâna. E pe un CanvasLayer 128, cu `PROCESS_MODE_ALWAYS`, și e desenată fără antialias, ca să meargă cu pixel art-ul.
- 🪤 Clicul care sare peste intro-ul meniului e **consumat** înainte să ajungă la `Gamepad._input`, fiindcă autoload-urile primesc `_input` ultimele. Soluția: `_anima_cursorul()` citește și `Input.is_mouse_button_pressed` în fiecare cadru.
- 🪤 **`tool_cursor.tscn` trebuie rulat în FEREASTRĂ**, nu headless: headless dă 10 eșecuri la ascunderea cursorului, identic și pe codul vechi (verificat). În testul de clic, evenimentele se trimit prin `Input.parse_input_event`, nu prin `Gamepad._input(...)`, altfel `is_mouse_button_pressed` rămâne fals și plasa de siguranță eliberează imediat clicul.
- ✅ `tool_cursor.tscn`: TOTUL E BINE. `scores.save` a rămas identic.
...
> ✅ **Rezolvat pe 2026-09-26** (Hooligan, al 7-lea): rândurile se strâng singure (`_celula_lista`, în `LISTA_H`), fără scroll. Vezi session log 2026-09-26b și 2026-09-26d.
...
**Fișiere noi:** `tool_cursor.gd` + `tool_cursor.tscn`.
**Atinse:** `player.gd` (`ARME["cross"]` + comentariile cu cifre), `gamepad.gd` (secțiunea **CURSORUL**, `_ready`, `_process`, `_schimba_mod`), `tool_cruce.gd`, `codex.html`, README.

### Crucea: 1.5/s → 1.0/s

`"interval": 1.0 / 1.0`. Cum turul E cadența, panoul scrie singur **1.00/s**, iar un inamic din inel încasează **2 lovituri pe secundă** (nu 3): 60 dps la pornire în loc de 90. `tool_cruce` a trecut cu **0 probleme** după trei reparații ale LUI, nu ale jocului:
- cifrele așteptate (1 tur/s, 2 lovituri/s, 2 ture/s cu cadența dublă);
- ⚠️ **`op_start` e pornit în salvarea lui Răzvan** și testul nu-l oprea → player-ul de test avea 100 damage, 2.50/s și 11 cruci. Acum îl oprește NUMAI în RAM și-l pune la loc în `_gata()`. Orice unealtă care măsoară statusuri de pornire trebuie să facă la fel (ca `GameSettings.upgrades = {}`);
- fereastra probei din lumea reală („omoară un inamic doar cu inelul") 12 s → 20 s, fiindcă inelul e mai lent.

### Cursorul

`menu/Mouse.png` (48×62, o mână care arată cu degetul) e cursorul, **vizibil numai într-un meniu**, ascuns cât joci și ascuns oriunde cu controllerul în mână. Stă în `gamepad.gd`, fiindcă acolo era deja singurul loc care atinge `Input.mouse_mode` — două locuri care se bat pe el ar fi clipit cursorul.

- 🔑 **„Meniu" = un `CanvasLayer` vizibil (`_straturi()`) care ține un panou DESCHIS cu butoane** (`_are_butoane`: coboară prin arbore, taie ramurile ascunse, se oprește la primul `BaseButton`). Fără listă de ecrane: unul nou primește cursorul singur.
- ⚠️ **Prima variantă („un buton pe care-l poți apăsa ACUM", adică `_primul_focusabil`) a picat pe ecrane reale:** omul în palton își ascunde butoanele cât dă cu zarul, EGT-ul le dezactivează cât se învârt rolele, meniul principal le ține ascunse în intro. Mâna ar fi clipit în mijlocul aceluiași meniu. Contează dacă se văd PĂRINȚII butonului, nu el.
- ⚠️ **Scurtătura din rundă:** cu jocul NEOPRIT și scena ne-`Control`, `in_meniu()` iese pe loc — toate ecranele cu butoane din `main.tscn` pun jocul pe pauză. **Deci un ecran nou cu butoane în rundă TREBUIE să pună `get_tree().paused = true`**, altfel n-are cursor (și oricum ar fi un bug: lumea ar merge sub el). ~0,5 µs pe apel.
- Vârful (hotspot) = **(16, 1)**, vârful arătătorului, măsurat pe poză (și verificat în test: pixel plin, rândul 0 gol). Pus pe `CURSOR_ARROW`, `CURSOR_POINTING_HAND`, `CURSOR_IBEAM`.
- Cursorul NU se întinde cu fereastra (îl desenează sistemul) — iese 48×62 pixeli de ecran la orice rezoluție.

### Verificat

`tool_cursor.tscn` (merge și headless, verifică `Input.mouse_mode`; pe o captură din Godot cursorul nu apare oricum): meniul principal → vizibil, pad → ascuns, clic → înapoi; intro-ul rundei → ascuns; joc → ascuns; **joc pus pe pauză fără niciun meniu → ascuns** (niciun strat nu ține butoane în repaus, deci cinematicile nu scot mâna); pauză, Alba-Neagra, EGT, omul în palton, level up → vizibil, închis → ascuns; ESC adevărat prin InputMap. Salvarea `scores.save` identică octet cu octet după ambele unelte (`cmp`). Codex: rând nou **Cross** în „Statusuri de start" (nu exista deloc), randat în Chrome (~1 MB, nu pagină albă). **Nerepublicat** pe claude.ai.
...
### ⚠️ Două lucruri de ținut minte ✅ *Rezolvat pe 2026-09-26: rândurile se strâng singure în `LISTA_H` — o armă în plus încape (vezi session log 2026-09-26d).*
...
`audio/Mage Staff Attack.wav`, făcut din DOUĂ sunete din `Soundpack/` stratificate ca `Dash.wav`: `DSGNImpt_EXPLOSION-Thud-004` (primele 130 ms, tăiat peste 800 Hz) = greutatea, `DSGNMisc_HIT-Spell Hit-004` (240 ms, tăiat sub 170 Hz, −4 dB la 3,2 kHz, −6 dB peste 7 kHz) = vraja. 48 kHz/16 biți, vârf −1 dBFS.

- Cerința „să nu fie deranjant, dar profesionist" **e o cerință de mixaj, nu de gust**: tragi de 2 ori pe secundă din start, iar urechea obosește cel mai repede la **3–8 kHz**. Rezultat măsurat: **bas −3,5 dBFS, 3–8 kHz −15,5 dBFS**, 240 ms, stins la ~200 ms. Am măsurat 18 familii candidate; niciuna singură nu mergea (`Spell Hit` n-are bas deloc, `Thud` n-are nimic de vrajă).
- **A ÎNLOCUIT `mage_shoot`**, nu s-a adăugat peste el. Cel vechi e brut din `Soundpack/` (96 kHz/24 biți, netrecut prin pipeline), are 1,43 s de conținut real la o cadență de 0,5 s — se călca de trei ori peste el însuși — și toată energia sub 200 Hz și **peste 8 kHz**, adică fix șuieratul care obosește. A rămas în `Audio.SFX`, nefolosit: se aduce înapoi punând numele și `-12.0` în `player.gd::_fire_bullets`.
- **Volumul −16 dB**: pare mai jos decât −12-ul vechi, dar fișierul nou e cu 5 dB mai gras (−17,2 dBFS RMS pe primele 250 ms față de −22,2), deci amândouă ies pe la **−33..−34 dBFS efectiv**. Scara: sabie −25, `enemy_hit` −31,5, **atacul mage −33,2**, pași −37.
