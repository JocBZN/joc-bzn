...
| **Cursed Sword** | No bullets — a **melee slash** in the direction you're **facing** (`_facing`), hitting *every* enemy in a forward cone (`sword_range` = 108 px, half-angle `sword_arc_dot` = 0.75 → ±42°) for `sword_base_damage + bullet_damage`. The cone is **measured from the art**, so you only hit what the slash visibly covers. Starts **slow** (`fire_interval × sword_slow_start` when picked) and speeds up with attack-speed upgrades. The slash animation is a **child of the player**, so it follows you (feels like the sword is always in hand). Scales with damage / crit / knockback / instakill / weapon-size upgrades. |
...
- ✅ **Slash nudged left & drawn under the player.** `sword_screen_offset` = (−20, 0) pushes the whole slash 20 px left **on screen**, the same whichever way you face (it is added *un-rotated*, unlike `sword_reach`/`sword_lateral` which live in art space and rotate with you) — the sword reads as held in the left hand. The hitbox moves with it: since the art shifts rigidly, its footprint around `global_position + sword_screen_offset` is unchanged, so the measured cone still holds and only its apex moves (`origine` in `_sword_swing`). The slash `z_index` is now **−1**, so the player always draws in front — previously, slashing north painted the arc over his head.
- ✅ **Cursed Sword hitbox now matches the art.** Every pixel of the 10 slash frames was measured through the same maths `_spawn_sword_slash` uses: the art reaches **107 px** and spans **±62°**, while the code was hitting at **135 px / ±81°** — roughly **3× the visible area**, killing enemies past your shoulders. Refitted to `sword_range` = 108, `sword_arc_dot` = 0.75 (±42°). Also fixed: an enemy standing *exactly* on you was never hit (`normalized()` of a zero vector → `dot` 0 → cone test always failed).
- ✅ **Slash is centred on the facing axis.** The art is drawn lopsided — its sweep ran from **−62°** (above the aim line) to only **+34°**. New `sword_lateral` = 12 px (perpendicular offset, applied in art space then rotated by facing) makes it sweep **−41°..+42°**. Note `sword_art_rotation` cannot fix this: it spins the sprite about its own centre, 62 px in front of you, so 24° of it moves the sweep by only 3.5°.
- ⚠️ **Balance follow-up:** the honest hitbox covers ~⅓ of the old area, so the sword hits noticeably less. Not compensated yet — knobs are `sword_base_damage`, `sword_scale` (re-measure the cone if changed) and `sword_slow_start`.
- ✅ **New weapon — Cursed Sword** (4th selectable weapon in the menu). Auto-**slashes in the facing direction** every `fire_interval`, hitting every enemy in a forward cone. Base swing is intentionally **slow** (`sword_slow_start` = 1.9× interval when picked) so attack-speed upgrades feel impactful; scales with the player's damage / crit / knockback / instakill / weapon-size upgrades (modelled on the Extinguisher aura + bullet instakill). Tunables on `player.gd`: `sword_range`, `sword_arc_dot`, `sword_lateral`, `sword_base_damage`, `sword_reach`, `sword_scale`, `sword_art_rotation`, `sword_slow_start` — note `sword_reach`/`sword_scale`/`sword_lateral` and `sword_range`/`sword_arc_dot` are a **package**: the last two are measured from the art with the first three fixed, so changing one means re-measuring all.
