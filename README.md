...
- ✅ **The slash looks identical in all 8 directions, just rotated**, and is drawn **under the player** (`z_index` −1 — previously, slashing north painted the arc over his head). Everything that positions the art now lives in **art space** and rotates with your facing, via one `_sword_offset(dir)` used by *both* the visual and the hitbox, so the two cannot drift apart. Pulling the slash 20 px closer is baked into `sword_reach` (62 → 42) rather than an un-rotated screen offset — a screen offset is what broke direction consistency in the first place (it drags the slash toward you at east but pushes it ahead at west).
- ✅ **Hitbox is a disc, not a cone.** At `sword_reach` 42 the art wraps around the player (its tail lands ~12 px *behind* him, hidden under his sprite), so a forward cone can no longer describe it — fitting one yields ±180°, i.e. hitting everything around you. It is now a disc of radius `sword_hit_radius` = 56 (the art's furthest pixel from its own centre) around `global_position + _sword_offset(dir)`; `sword_range` and `sword_arc_dot` are gone. Measured against the sweep envelope (union of the 10 frames = 3,552 px²), the disc is **36%** real slash vs the cone's 42% — and vs **14%** for the original broken hitbox.
...
- ✅ **Slash is centred on the facing axis.** The art is drawn lopsided — its sweep leaned ~14° above the aim line, so it read as slashing past the enemy. `sword_lateral` = 12 px (perpendicular offset, in art space, so it rotates with you) balances it. Note `sword_art_rotation` *cannot* fix this: it spins the sprite about its own centre, well in front of you, so even 24° of it moved the sweep by only 3.5°.
...
- ✅ **New weapon — Cursed Sword** (4th selectable weapon in the menu). Auto-**slashes in the facing direction** every `fire_interval`. Base swing is intentionally **slow** (`sword_slow_start` = 1.9× interval when picked) so attack-speed upgrades feel impactful; scales with the player's damage / crit / knockback / instakill / weapon-size upgrades (modelled on the Extinguisher aura + bullet instakill). Tunables on `player.gd`: `sword_reach`, `sword_lateral`, `sword_hit_radius`, `sword_scale`, `sword_base_damage`, `sword_art_rotation`, `sword_slow_start` — note `sword_reach`/`sword_lateral`/`sword_scale` and `sword_hit_radius` are a **package**: the radius is measured from the art with the other three fixed, so changing one means re-measuring it.
