...
| **Cursed Sword** | No bullets — a **melee slash** in the direction you're **facing** (`_facing`), hitting *every* enemy inside a disc centred on the slash itself (`_sword_offset(dir)`, 42 px ahead of you), for `sword_base_damage + bullet_damage`. Built on the **Firewalker model**: `sword_size` is the width in px and the hit radius is *derived* from it (`× sword_hit_ratio`), so art and hitbox cannot drift apart. Starts **slow** (`fire_interval × sword_slow_start` when picked) and speeds up with attack-speed upgrades. The slash animation is a **child of the player**, so it follows you (feels like the sword is always in hand), and draws **under** him. Scales with damage / crit / knockback / instakill / weapon-size upgrades. |
...
- ✅ **Cursed Sword rebuilt on the Firewalker model** (`firetrail.gd`), with new art. Three things copied from it, and they fix the whole saga below:
  ...
  - **Hitbox derived from size** — `radius = sword_size × sword_hit_ratio` (0.47), like Firewalker's `radius = size * 0.4`. The ratio is the art's furthest pixel (30.1) over the frame width (64), so it stays right at *any* size. **The art and the hitbox can no longer drift apart** — that was the bug that started all of this.
  ...
  - Knobs: `sword_size`, `sword_reach`, `sword_lateral`, `sword_hit_ratio`, `sword_art_rotation`, `sword_anim_speed`, `sword_debug`. Set **`sword_debug = true`** to draw the hitbox live over the game — red circle = what hits, blue cross = where the art sits, white line = facing.
...
- ✅ **Hitbox is a disc, not a cone.** With the slash sitting close in front, its art wraps around the player, so a forward cone cannot describe it — fitting one yields ±180°, i.e. hitting everything around you. The disc also makes an enemy standing *exactly* on you hittable by construction; the old cone never hit that case (`normalized()` of a zero vector → `dot` 0 → the test always failed).
- ✅ **The hitbox is measured from the art, not guessed.** It originally hit at 135 px in a ±81° cone while the art only reached 107 px within ±62° — roughly **3× the visible area**, killing enemies past your shoulders. Fit quality since (share of the hitbox that is really slash, measured against the sweep envelope): broken cone **14%** → fitted cone **42%** → disc with the old art **36%** → disc with the new, thinner art **29%**. Tighten `sword_hit_ratio` if you want it stingier.
